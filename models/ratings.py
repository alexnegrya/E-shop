...
class Rating(Model):
    ...
    def validate_model_field(self, name: str, value):
        if name == 'stars':
            ...
            if value not in range(1, 11): raise ValueError(
                'stars must be in int range 1-10')
        elif name == 'review':
            if type(value) != str: raise TypeError('review must be str type')
            if len(value) > 500: raise ValueError(
                f'max {name} length is 500 symbols')
            ...
            value = f'"{value}"'
        elif name.endswith('_id') and type(value) != int: raise TypeError(
